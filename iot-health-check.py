...
domoticz_devices_low_battery = jstyleson.loads(
    config["DOMOTICZ"]["DEVICES_LOW_BATTERY"]
)
...
def get_health_checks():
    ...
    try:
        response = requests.get(
            config["HEALTH_CHECKS"]["API_ALL_CHECKS"],
            headers={"X-Api-Key": config["HEALTH_CHECKS"]["X-API-KEY"]},
            timeout=60,
        )
        checks = [
            check for check in response.json()["checks"] if check["status"] != "up"
        ]
    except:
        pass
...
def get_monit_services():
    ...
    response = requests.get(
        config["MONIT"]["API_STATUS"], verify="/root/.local/share/mkcert/rootCA.pem"
    )
    doc = etree.fromstring(response.content)

    services = [
        {"status": "??", "name": service.find("name").text}
        for service in doc.iterfind('service/[@type="3"]')
        if service.find("monitor").text == "1" and service.find("status").text != "0"
    ]
...
def get_zway_devices():
    ...
    response = requests.get(
        config["ZWAY"]["API_ALL_DEVICES"],
        headers={"ZWaySession": config["ZWAY"]["ZWAYSESSION"]},
    )
...
def send_mail(report):
    ...
    server = smtplib.SMTP(config["MAIL"]["SERVER"], config.getint("MAIL", "PORT"))
    server.ehlo()
    server.starttls(context=context)
    server.ehlo()
    server.login(config["MAIL"]["FROM"], config["MAIL"]["PASSWORD"])
    server.sendmail(config["MAIL"]["FROM"], config["MAIL"]["TO"], msg.as_string())
    server.quit()
...
def create_context():
    ...
    context = {
        "devices_inactive": sorted(devices_inactive, key=lambda d: d["LastUpdate"]),
        "devices_low_battery": devices_low_battery,
        "devices_no_roomplan": sorted(devices_no_roomplan, key=lambda d: d["idx"]),
        "log_errors": sorted(log_errors, key=lambda e: e["count"], reverse=True),
        "health_checks": health_checks,
        "monit_services": monit_services,
        # "zway_devices_failed": zway_devices_failed,
        # "zway_devices_low_battery": zway_devices_low_battery,
    }
